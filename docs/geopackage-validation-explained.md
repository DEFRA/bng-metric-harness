...
| `IGGIS_OUTSIDE_REDLINE` — *spatial-problems/Baseline - iggi outside.gpkg* | Green infrastructure area escaping past the boundary. Believed unreachable in ordinary use: that layer is not in the template, so a file carrying it is rejected as an unexpected layer first. Tolerance 0.5 m². The user sees a placeholder. |
