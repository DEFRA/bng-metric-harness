...
| Need | How |
| --- | --- |
| The site file | `hs2-phase2a-subsection/Layers/BNG Service Layers.gpkg`, already generated. `python3 development/scale-test-generator/generate.py` rebuilds it in 7 seconds |
...
> **Superseded in part, 18 September 2026.** Button 1 treated a
> post-intervention feature that had been removed on purpose as a gap and
> copied it back, silently undoing the removal; button 4 did the same when
> creating rows. Both now name what they are about to put back and ask first.
> Windows measured the copy at 6.1 seconds for 11,554 parcels, of which 5.07
> seconds was the table-view update, which now drives the progress dialog
> rather than blocking behind a bar stopped at 90%. The buttons have also lost
> their list numbers, so the names in this section read `Copy baseline to
> post-intervention` and so on. The numbers implied an order the menu never
> had: it lists buttons in the order the project stores them, which was 2, 4,
> 1. They are now stored in the order a user needs them.
...
> **Superseded in part, 18 September 2026.** Windows testing found that the
> conditions written here carried the template's list number, as
> `4. Fairly Poor`, which the workbook's own lookups cannot match: every
> habitat and watercourse row reported "Check Data" and both modules totalled
> zero units. Hedgerows were unaffected, because their reference list carries
> no numbers. Watercourse retention categories carried the same numbering, so
> no watercourse row reached the retained, enhanced or enhancement columns
> either. Both are fixed, the practice section now fills 4,736 cells rather
> than 4,628, and its C-1 holds 25 rows with 11 on C-3. The timings below are
> also superseded: filling a sheet searched it once per cell, which is
> quadratic, and one pass instead took the full site from 70 seconds to about
> 6.
...
```sh
python3 development/scale-test-generator/generate.py --fraction 0.12
```
...
```sh
python3 development/scale-test-generator/verify/claim2_nothing_altered.py
