...
- **Post-intervention coverage is thin.** Most post-intervention fixtures carry only Habitats + RLB; the exceptions are `valid/Post-intervention - retained watercourse.gpkg` and `valid/Post-intervention - all unit and intervention types.gpkg`. Further fixtures covering hedgerows, watercourses and trees exist, but only in `journey-tests/test/example-files/`.
- **No post-intervention fixture has a baseline that shares its RLB and covers every fate.** `valid/Post-intervention - all unit and intervention types.gpkg` is generated by synthetic mode, which emits a single post-intervention-shaped file rather than a pair, so it has no baseline half. The `retained *` pairs and `bng-500/` model the two-stage journey but at a single fate or out of scope.
