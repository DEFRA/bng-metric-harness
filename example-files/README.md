...
| `Baseline - tiny gap between parcels.gpkg`    | Two parcels tile the RLB except a 0.32 m² unsnapped gap where they meet the boundary — the classic digitising sliver. Below the 0.5 m² `AREA_SUM_MISMATCH` tolerance, so the file is **accepted**: the positive control proving hairline gaps between parcels no longer fail validation (the removed `SLIVERS_INSIDE_REDLINE` check would have rejected it). Flaw: `tiny-gap`. |
...
- **Positive controls for the geometry validators are thin.** Most geometric flaws have a negative fixture and no valid twin — near-miss positives (parcels sharing an exact boundary, a hedgerow ending precisely on the RLB, a site just under the area limit) would catch a validator that fires unconditionally. `valid/Baseline - tiny gap between parcels.gpkg` is the first: a sub-tolerance gap the area checks must accept.
...
- **No post-intervention fixture for `AREA_PARCELS_TOO_SMALL`.** The old `Post-intervention - slivers.gpkg` was dropped when the derived-sliver check was replaced by the too-small-parcel check: the generator can only apply `--flaw parcel-too-small` to a baseline-shaped synthetic file, not to a post-intervention one, so a faithful post-intervention twin cannot currently be produced. `Baseline - parcel too small.gpkg` covers the rule.
- **Two `valid/` post-intervention fixtures predate the created-linear fix (BMD-891).** Their created hedgerows and watercourses carry `Retention Category = Lost`, which the backend strips from the hedgerow, watercourse and tree layers — the `Lost` → `Created` mapping applies to area habitats only — so they render from the file but are dropped on upload. `bng-library` now persists `Created` with the NE template's `To be created` / `N/A` baseline columns, which the backend accepts and calculates units from, and the four baseline fixtures that carried the same rows have been regenerated. These two could not be:
  - `Post-intervention - all unit and intervention types.gpkg` — needs `BMD-845-synthetic-created-watercourses`, without which a regenerated file loses the created watercourse that is this fixture's reason for existing.
  ...
  Both need a coordinated refresh once the `bng-library` pin has moved. See `docs/ne-template-linear-retention.md`.
...
| **Small, all five layers**  | 1–5 habitats, 1–3 hedgerows, 1–2 rivers, 1–5 trees | all of `spatial-problems/` (baseline), `attribute-problems/Baseline - duplicate habitat ref.gpkg`, `valid/Baseline - tiny gap between parcels.gpkg` |
...
Reproducibility splits along family, not shape: `spatial-problems/`, `empty-layer/`,
`attribute-problems/` and `valid/Baseline - tiny gap between parcels.gpkg` map onto
`--flaw` names and can be regenerated. **No `invalid-schema/`
