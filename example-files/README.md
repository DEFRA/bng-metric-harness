...
| `Baseline - watercourses retained.gpkg` / `Post-intervention - watercourses retained.gpkg` | Purpose-built for the **retained watercourse habitat-details page** (BMD-724). Generated by the permutations runner's `intervention-watercourse-retained` scenario with `--seed 724` (reproducible). All five layers (6 habitats, 3 hedgerows, 2 rivers, 5 trees, 1 RLB); the pair shares one RLB and passes `scripts/check-gpkg-pair.mjs`. **`R001`** is the subject: a Retained **Canals** watercourse, Medium distinctiveness, Fairly Good condition, **Low (1)** strategic significance, No Encroachment on both water and riparian, 0.85 km — every field the details page shows is populated and units are non-zero (proposed mirrors baseline). `R002` is an Enhanced watercourse for contrast. Reproduce with `npm run generate:gpkg -- --permutations --only intervention --seed 724`. |
