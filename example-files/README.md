...
| `Baseline - created linear features.gpkg` / `Post-intervention - created linear features.gpkg` | Purpose-built for the created-linear-features fix (BMD-891), generated from the fixed `bng-library`. All five layers (55 habitats, 18 hedgerows, 3 rivers, 27 trees, 1 RLB). The post-intervention hedgerows carry every retention category (Created 3, Enhanced 6, Lost 6, Retained 3) and the rivers carry one of each (Created / Lost / Retained); every `Created` linear row uses the NE template's `To be created` / `N/A` baseline columns, so it survives upload and produces units, while the `Lost` rows are still dropped — demonstrating **AC1** (created hedgerow/watercourse survives) and **AC2** (removed one still ignored, told apart) together. Distinctiveness is in scope; the pair shares one RLB and passes `scripts/check-gpkg-pair.mjs`. **Urban Trees carry no `Created` row — the generator cannot emit one yet; see Gaps.** |
...
- **Two `valid/` post-intervention fixtures predate the created-linear fix (BMD-891).** Their created hedgerows and watercourses carry `Retention Category = Lost`, which the backend strips from the hedgerow, watercourse and tree layers — the `Lost` → `Created` mapping applies to area habitats only — so they render from the file but are dropped on upload. `bng-library` now persists `Created` with the NE template's `To be created` / `N/A` baseline columns, which the backend accepts and calculates units from, and the four baseline fixtures that carried the same rows have been regenerated. These two could not be:
  ...
  Both still await a coordinated refresh. See `docs/ne-template-linear-retention.md`. A separate purpose-built `valid/Baseline - created linear features.gpkg` / `Post-intervention - created linear features.gpkg` pair now gives positive, post-fix upload coverage of created **hedgerows and watercourses** (created + lost of each in one file), independent of those two refreshes.
- **No fixture — and no generator path — covers a created individual tree.** BMD-891 added the `Newly Planted` tree category to `bng-library` (`retention.mjs`), but the synthetic Urban Trees builder draws retention from `Retained / Enhanced / Lost` only (`generateUrbanTrees`), so it never emits a `Created` tree — hence the `created linear features` pair above covers hedgerows and watercourses but not trees. A created-tree fixture needs a small generator change: seed one tree `Created` deterministically, mirroring the seeded created river at Rivers index 2.
