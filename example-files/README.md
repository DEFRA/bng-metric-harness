...
| `Baseline - duplicate habitat ref.gpkg`                 | Two habitat rows share a reference. **Valid since BMD-1001** — the duplicate-ref check was removed, so this file uploads cleanly and is kept to prove that. | `duplicate-habitat-ref`        | — |
...
- **Is `valid/Baseline - complete but null area refs.gpkg` doing what its name says?** Its `Habitats."Parcel Ref"` values are the three-character string `"Null"`, not SQL `NULL`; every other column matches the "complete with area refs" fixture exactly. Two problems follow. If the intent was to cover *absent* refs, the fixture does not do it and a genuine SQL-`NULL` fixture is needed. The second half of the question is now settled: all three rows sharing the value used to trip `DUPLICATE_HABITAT_REF`, but BMD-1001 removed that check — `Null` is precisely the literal the NE template writes into an untouched cell — and the file validates clean, so `valid/` is where it belongs.
