...
- **The full-coverage fixture predates the created-linear fix.** Its created hedgerows and watercourses carry `Retention Category = Lost`, which the backend strips from the hedgerow, watercourse and tree layers — the `Lost` → `Created` mapping applies to area habitats only — so they render from the file but are dropped on upload. The generator no longer writes them that way: `bng-library` now persists `Created` with the NE template's `To be created` / `N/A` baseline columns, which the backend accepts and calculates units from. Regenerating `valid/Post-intervention - all unit and intervention types.gpkg` is blocked on two things landing: the `bng-library` pin moving past that fix, and `BMD-845-synthetic-created-watercourses`, without which a regenerated file would lose the created watercourse that is this fixture's reason for existing. See `docs/ne-template-linear-retention.md`.
