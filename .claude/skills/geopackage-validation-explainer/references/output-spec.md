...
- **Never write a number.** State a threshold by naming its constant in the `tolerance` field instead; the build extracts the value from the code, formats it in the reader's units and appends the sentence. Optionally set `toleranceIs` to change the wording from the default "Tolerance". This is the one rule here that is enforced rather than advised: a hand-written figure is the most rot-prone thing a description can contain, because tolerances live in constants that no error message mentions, so nothing else in the skill would notice one moving. A renamed constant fails the build rather than publishing a stale figure.
- **Do say what passes** where the threshold needs interpreting — that parcels sharing an edge are the normal case, or that a line tolerance is a total across every excursion rather than an allowance per excursion. The generated sentence supplies the number; the description supplies the meaning.
