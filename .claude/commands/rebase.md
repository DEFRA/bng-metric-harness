...
argument-hint: backend|frontend|library|journey-tests|prototype|perf-tests
---

Rebase the named sibling's current branch onto the latest `origin/main`.

The argument selects the target repo: `backend` → `../bng-metric-backend`, `frontend` → `../bng-metric-frontend`, `library` → `../bng-library`, `journey-tests` → `../bng-metric-journey-tests`, `prototype` → `../bng-metric-digital-prototype`, `perf-tests` → `../bng-perf-tests`. If no argument is given, ask which repo to target — do not guess.
